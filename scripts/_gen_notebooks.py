...
nb2 = [
 ...
 code("TRAIN_JSON   = 'train/train.json'",
      ...
      "DEV_TABLES = f'{DEV_DIR}/dev_tables.json'   # same schema format as train",
 ...
 md("## 4. Preprocess → JSONL",
    "Both splits build the prompt schema from `tables.json` (so train and eval use",
    "the *same* schema format — no train/serve mismatch). Dev also gets `--db_root`",
    "so the predicted SQL can be executed against the real databases."),
 code("!python -m src.data_prep --source bird --json {TRAIN_JSON} --tables_json {TRAIN_TABLES} \\",
      ...
      "!python -m src.data_prep --source bird --json {DEV_JSON} --tables_json {DEV_TABLES} --db_root {DEV_DB} \\",
